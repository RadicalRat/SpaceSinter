...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("estimate_sun_pose::EstimateSunPoseBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "EstimateSunPose", BT::NodeConfiguration()));
}
