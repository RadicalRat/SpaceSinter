 ...
 * @brief TODO(...)
 */
class EstimateSunPose : public moveit_studio::behaviors::SharedResourcesNode<BT::SyncActionNode>
{
public:
  /**
   * @brief Constructor for the estimate_sun_pose behavior.
   ...
   */
  EstimateSunPose(const std::string& name, const BT::NodeConfiguration& config, const std::shared_ptr<moveit_studio::behaviors::BehaviorContext>& shared_resources);

  
  /**
   * @brief Implementation of the required providedPorts() function for the estimate_sun_pose Behavior.
   ...
   * @return estimate_sun_pose does not expose any ports, so this function returns an empty list.
   */
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for EstimateSunPose.
   * @details This function is where the Behavior performs its work when the behavior tree is being run. Since EstimateSunPose is derived from BT::SyncActionNode, it is very important that its tick() function always finishes very quickly. If tick() blocks before returning, it will block execution of the entire behavior tree, which may have undesirable consequences for other Behaviors that require a fast update rate to work correctly.
   */
