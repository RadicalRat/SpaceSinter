 ...
 * @brief TODO(...)
 */
class TestBehavior : public moveit_pro::behaviors::SharedResourcesNode<BT::StatefulActionNode>
{
public:
  /**
   * @brief Constructor for the test_behavior behavior.
   ...
   */
  TestBehavior(const std::string& name, const BT::NodeConfiguration& config, const std::shared_ptr<moveit_pro::behaviors::BehaviorContext>& shared_resources);

  
  /**
   * @brief Implementation of the required providedPorts() function for the test_behavior Behavior.
   ...
   * @return test_behavior does not expose any ports, so this function returns an empty list.
   */
   ...
   * @return Always returns BT::NodeStatus::RUNNING, since the success of Behavior's initialization is checked in @ref onRunning().
   */
